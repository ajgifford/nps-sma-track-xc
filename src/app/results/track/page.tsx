...
import { getTrackMeetsGroupedByYear, getTrackYears } from '@/lib/trackMeets';
...
export default function TrackResults() {
  const meetsGroupedByYear = getTrackMeetsGroupedByYear();
  const years = getTrackYears(); // sorted newest to oldest
          ...
          <div key={year} className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {meets.map((meet, index) => {
                const hasResults = meet.hasResults !== false; // Default to true if not specified
                const isFuture = isFutureDate(meet.date);
                const canViewResults = hasResults && !isFuture;
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="mt-4 text-sm text-gray-500 italic">
                          {isFuture ? '📅 Upcoming event' : '⏳ Results not yet available'}
                        </div>
                      </div>
                      <div className="text-3xl font-bold text-gray-300">
                        {index + 1}
